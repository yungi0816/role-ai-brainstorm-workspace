...
| Backend | Express API, SQLite 저장, Provider router, AI JSON 정규화, 마인드맵 patch 적용, conversation export API |
...
| 4 | 마인드맵 노드 편집 | AI가 만든 노드를 사용자가 직접 고치면 실사용성이 올라간다. |
| 5 | installer 아이콘/서명 | 공개 배포 프로젝트처럼 보이는 완성도가 올라간다. |
| 6 | patch service 단위 테스트 | AI 출력 방어 로직의 신뢰도를 코드로 증명할 수 있다. |
